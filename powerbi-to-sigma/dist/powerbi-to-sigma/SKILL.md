...
**Smoke-tested 2026-05-31** against the KitchenSink (model `049863fa` / DM `a564d6d5`): extract normalized all 9 visuals; build emitted 3 masters + 9 chart elements (line correctly single-series, pivot with `rowsBy`/`values`) with collision-free ids; `validate-spec.rb` → 0 errors.

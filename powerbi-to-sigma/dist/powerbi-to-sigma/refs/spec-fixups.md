...
Charts post fine with no `layout` (they stack vertically) — but that's not done. Apply the 24-col grid via a single top-level `layout` XML string and `put-layout.rb`. See `refs/vendored/powerbi-visual-layout.md §4` for the px→grid math and `refs/vendored/workbook-layout.md` for the XML shape (`<Page>`/`<LayoutElement>`/`<GridContainer>`).
